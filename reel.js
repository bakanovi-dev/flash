[
  {
    "id": 1,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой, бывший управляющий хедж-фондом) рассказывает зрителю закадровым голосом про карьерный путь финансиста — этап, когда деньги начинают расти.",
    "quote_en": "You start earning some big bucks... which only serves to make you aware of the even bigger bucks you could be earning.",
    "quote_ru": "Ты начинаешь зарабатывать большие деньги — и это только заставляет тебя осознать, какие ещё большие деньги ты мог бы зарабатывать.",
    "expressions": [
      {
        "phrase": "Big bucks",
        "literal": "Большие самцы оленя",
        "explanation": "В Америке XVIII в. оленьи шкуры (buckskin) были валютой на фронтире. Buck стал сленгом для доллара, а big bucks — большие деньги."
      },
      {
        "phrase": "It only serves to...",
        "literal": "Это лишь служит тому, чтобы...",
        "explanation": "Устойчивая конструкция с ироничным оттенком: «это только приводит к…» (обычно к парадоксальному эффекту)."
      }
    ]
  },
  {
    "id": 2,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) закадровым голосом описывает зрителю первый этап взрослой жизни — выпуск из колледжа и брак, когда у пары ничего нет.",
    "quote_en": "You're dead broke, but there's two of you now.",
    "quote_ru": "Вы оба нищие, но теперь вас уже двое.",
    "expressions": [
      {
        "phrase": "Dead broke",
        "literal": "Мёртво сломан",
        "explanation": "Broke «без денег» — от итал. banca rotta («сломанная скамья» разорившегося торговца, отсюда и bankrupt). Dead — усилитель «полностью» (dead tired, dead serious)."
      }
    ]
  },
  {
    "id": 3,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) закадровым голосом объясняет зрителю стадию, когда у успешного финансиста уже есть дом, машина и долги под них.",
    "quote_en": "You're leveraged, which is a fancy way of saying you've got something to show for your efforts.",
    "quote_ru": "Ты «в плече» — это красивый способ сказать, что у тебя есть что показать за свои труды.",
    "expressions": [
      {
        "phrase": "Leveraged",
        "literal": "Использующий рычаг",
        "explanation": "От lever — рычаг (Архимед). В финансах — купить большой актив на заёмные деньги, используя свой капитал как «рычаг». Часто = «закредитован под залог»."
      },
      {
        "phrase": "A fancy way of saying",
        "literal": "Красивый способ сказать",
        "explanation": "Намёк, что простую вещь обернули умным словом: «выпендрёжно сказанное то-то»."
      },
      {
        "phrase": "To have something to show for",
        "literal": "Иметь что показать за…",
        "explanation": "Идиома: иметь конкретный результат за вложенные усилия. Часто в негативе: I have nothing to show for it."
      }
    ]
  },
  {
    "id": 4,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) закадровым голосом описывает зрителю «вершину», к которой стремятся все жители его богатого пригорода Westmont Village.",
    "quote_en": "That hallowed plateau where, for the first time, you can actually afford your own life.",
    "quote_ru": "То заветное плато, на котором ты впервые по-настоящему можешь позволить себе свою собственную жизнь.",
    "expressions": [
      {
        "phrase": "Hallowed",
        "literal": "Освящённый, святой",
        "explanation": "Архаичное слово из молитвы «Отче наш» (Hallowed be Thy name). Используется для торжественности — «священный, заветный»."
      },
      {
        "phrase": "To afford your own life",
        "literal": "Позволить себе свою жизнь",
        "explanation": "Игра слов: обычно afford a house/car, а здесь — «позволить себе саму жизнь». Намёк, что многие живут не по средствам."
      }
    ]
  },
  {
    "id": 5,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) закадровым голосом перечисляет зрителю стандартный набор финансовых решений среднего успешного американца.",
    "quote_en": "You buy a house, 30% down, 30-year jumbo at four and a half percent.",
    "quote_ru": "Ты покупаешь дом: 30% первый взнос, 30-летняя джамбо-ипотека под 4,5%.",
    "expressions": [
      {
        "phrase": "30% down",
        "literal": "30% вниз",
        "explanation": "Down payment — первоначальный взнос. Down — потому что сумму «кладёшь сразу, на стол», в отличие от ежемесячных платежей."
      },
      {
        "phrase": "Jumbo (mortgage)",
        "literal": "Огромный",
        "explanation": "Ипотека выше федерального лимита (в США ≈ $766k). Слово от слона Jumbo из цирка П.Т. Барнума в 1880-х — стало нарицательным для «гигантского»."
      }
    ]
  },
  {
    "id": 6,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) закадровым голосом подводит итог финансовой картины своей жизни после развода с женой Мел и недавнего увольнения из хедж-фонда.",
    "quote_en": "A bank account that was still in the red from the divorce.",
    "quote_ru": "Банковский счёт, который всё ещё в минусе после развода.",
    "expressions": [
      {
        "phrase": "In the red",
        "literal": "В красном",
        "explanation": "Бухгалтеры до компьютеров писали убытки красными чернилами, прибыль — чёрными. Отсюда же in the black и Black Friday (день, когда магазины выходят в прибыль)."
      }
    ]
  },
  {
    "id": 7,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) закадровым голосом перечисляет свои финансовые обязательства, на фоне которых он ищет выход после потери высокооплачиваемой работы.",
    "quote_en": "A maxed out home equity line, tuition, property taxes...",
    "quote_ru": "Кредитная линия под дом, выбранная подчистую, обучение, налог на недвижимость…",
    "expressions": [
      {
        "phrase": "Maxed out",
        "literal": "Доведённый до максимума",
        "explanation": "Фразовый глагол to max out — выбрать кредитный лимит до конца. Чаще всего про карту: I maxed out my credit card."
      },
      {
        "phrase": "Home equity line",
        "literal": "Линия под собственный капитал в доме",
        "explanation": "Equity — твоя «доля» в доме за вычетом ипотеки. Банк даёт под неё возобновляемую кредитную линию (HELOC)."
      }
    ]
  },
  {
    "id": 8,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой, только что уволенный из хедж-фонда) сидит в баре с другом-финансистом Барни и пытается понять, на сколько ему хватит сбережений.",
    "quote_en": "I just need to know how long I can float on what I got.",
    "quote_ru": "Мне нужно понять, насколько меня хватит на том, что есть.",
    "expressions": [
      {
        "phrase": "To float on something",
        "literal": "Плыть на чём-то",
        "explanation": "Финансовый сленг: продержаться на имеющихся средствах, не утонув. От буквального — держаться на плаву. Также to float a loan."
      }
    ]
  },
  {
    "id": 9,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Барни (друг-финансист) в баре отвечает Купу (только что уволенному из хедж-фонда) на вопрос, на сколько хватит его сбережений. Прикинул его обязательные ежемесячные расходы и выдаёт прогноз.",
    "quote_en": "Your nut? Six months. Seven tops.",
    "quote_ru": "Твои месячные расходы? Шесть месяцев. Максимум семь.",
    "expressions": [
      {
        "phrase": "Your nut",
        "literal": "Твой орех",
        "explanation": "Циркачский сленг XIX в.: шериф снимал с повозки шапито гайку (nut) и возвращал, только когда труппа покрывала обязательные расходы города. Отсюда — твой минимум на жизнь."
      },
      {
        "phrase": "Tops",
        "literal": "Максимум, потолок",
        "explanation": "Сленговое сокращение: seven tops = «семь, и не больше». Идёт после числа: 50 bucks tops."
      }
    ]
  },
  {
    "id": 10,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Джек (босс хедж-фонда) — Купу (главному герою) в кабинете, где его только что уволили из-за слухов о связи с женой клиента. Озвучивает условия выходного пакета.",
    "quote_en": "They'll give you six months' draw if you don't sue.",
    "quote_ru": "Они дадут тебе полугодовую выплату, если не будешь судиться.",
    "expressions": [
      {
        "phrase": "Draw",
        "literal": "Вытянутое",
        "explanation": "Регулярный «выем» из общего пула: зарплата партнёра или аванс продавца под будущие комиссии. От to draw money — снимать деньги."
      }
    ]
  },
  {
    "id": 11,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) — Джеку (боссу), услышав предложенные условия увольнения из хедж-фонда. Защищает свой капитал, который он накопил в фонде за годы работы.",
    "quote_en": "My capital account? That's my money, Jack. No way.",
    "quote_ru": "Мой капитальный счёт? Это мои деньги, Джек. Ни в коем случае.",
    "expressions": [
      {
        "phrase": "No way",
        "literal": "Никак",
        "explanation": "Эмфатический отказ. От there's no way — «нет такого пути/способа». Современное идиоматическое: «ни за что»."
      }
    ]
  },
  {
    "id": 12,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой, недавно уволенный из хедж-фонда) — Барни (другу-финансисту), объясняя, почему не может сразу уйти к конкурентам и забрать своих клиентов.",
    "quote_en": "I have a two-year non-solicit.",
    "quote_ru": "У меня двухлетний запрет на переманивание клиентов.",
    "expressions": [
      {
        "phrase": "Non-solicit",
        "literal": "Не-приставание",
        "explanation": "Сокращение от non-solicitation agreement. To solicit — настойчиво обращаться, в т.ч. переманивать. Стандартный пункт в контрактах: уволившись, не имеешь права забирать клиентов."
      }
    ]
  },
  {
    "id": 13,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 2,
    "context": "Куп (главный герой) закадровым голосом описывает зрителю элитный загородный клуб Westmont, центр светской жизни его богатого пригорода, и принятые там наценки.",
    "quote_en": "40% above market for every salad, burger, coffee and cocktail they serve.",
    "quote_ru": "На 40% выше рынка за каждый салат, бургер, кофе и коктейль, что у них подают.",
    "expressions": [
      {
        "phrase": "Above market",
        "literal": "Выше рынка",
        "explanation": "Финансовый термин: цена выше рыночной нормы. Антоним — below market. Часто звучит при сделках с недвижимостью или зарплатами топ-менеджеров."
      }
    ]
  },
  {
    "id": 14,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 2,
    "context": "Бен Кон (управляющий конкурирующего хедж-фонда) — Купу (главному герою) на собеседовании, куда тот пришёл после увольнения. Объясняет принцип оплаты в новой фирме.",
    "quote_en": "We eat what we kill, but often I will partner a manager with a specific target.",
    "quote_ru": "Мы едим то, что сами убили, но часто я соединяю менеджера с конкретной целью.",
    "expressions": [
      {
        "phrase": "Eat what you kill",
        "literal": "Ешь то, что убил",
        "explanation": "Из охотничьего быта первобытных племён. В корпоративном жаргоне 80-х — модель оплаты: получаешь только то, что сам заработал, никаких окладов. Типично для юрфирм и хедж-фондов."
      }
    ]
  },
  {
    "id": 15,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 4,
    "context": "Куп (главный герой) — финансовому консультанту по телефону, преуменьшая серьёзность своих денежных проблем после увольнения и кражи драгоценностей у соседей.",
    "quote_en": "Yeah, it's just a little cash crunch. We're fine.",
    "quote_ru": "Да, просто небольшой дефицит наличности. Мы в порядке.",
    "expressions": [
      {
        "phrase": "Cash crunch",
        "literal": "Денежный хруст / сжатие",
        "explanation": "Crunch — критический момент, прижимающее давление. Cash crunch — временная нехватка живых денег при наличии активов. Похожее: credit crunch (кризис 2008 г.)."
      }
    ]
  },
  {
    "id": 16,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 5,
    "context": "Куп (главный герой) закадровым голосом размышляет о том, как от мелких краж у соседей он перешёл к более крупным и опасным целям.",
    "quote_en": "Higher risk, higher yield. Or maybe I was just starting to like it.",
    "quote_ru": "Выше риск — выше доходность. А может, мне просто начинало это нравиться.",
    "expressions": [
      {
        "phrase": "Higher risk, higher yield",
        "literal": "Выше риск — выше доход",
        "explanation": "Базовая аксиома финансов (от Гарри Марковица, нобелевка 1990 г.): чтобы заработать больше, надо рисковать больше. Yield буквально — «выход» (как урожая)."
      }
    ]
  },
  {
    "id": 17,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 2,
    "context": "Куп (главный герой) закадровым голосом объясняет зрителю, почему кражи у богатых соседей оказались выгоднее работы в хедж-фонде.",
    "quote_en": "It was hard to argue with that kind of tax-free return on investment.",
    "quote_ru": "Сложно было спорить с такой безналоговой доходностью инвестиций.",
    "expressions": [
      {
        "phrase": "Return on investment (ROI)",
        "literal": "Возврат на инвестицию",
        "explanation": "Ключевая метрика инвестора. Return — «возврат, отдача». Аббревиатура ROI стала культурным маркером корпоративной речи 90-х."
      },
      {
        "phrase": "Hard to argue with",
        "literal": "Сложно поспорить с",
        "explanation": "Идиома: что-то настолько убедительно, что возражений нет. Ироничное согласие."
      }
    ]
  },
  {
    "id": 18,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 9,
    "context": "Куп (главный герой) — Кэт (своему адвокату), когда обсуждает варианты сделки со следствием. Размышляет о том, как профессиональный навык применить к собственной ситуации.",
    "quote_en": "I've spent the last 20 years of my life hedging against risk.",
    "quote_ru": "Я последние 20 лет жизни занимался хеджированием рисков.",
    "expressions": [
      {
        "phrase": "To hedge against",
        "literal": "Огораживаться живой изгородью от",
        "explanation": "Hedge — живая изгородь вокруг участка, защита. С XVI в. в Англии стало означать «страховаться от потерь». Хедж-фонды занимаются именно этим (в теории)."
      }
    ]
  },
  {
    "id": 19,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 9,
    "context": "Кэт (адвокат) — Купу (главному герою), которого арестовали по подозрению в убийстве. Объясняет ему стратегию признания вины как способ минимизировать ущерб.",
    "quote_en": "The plea bargain is your hedge.",
    "quote_ru": "Сделка со следствием — твой хедж.",
    "expressions": [
      {
        "phrase": "Plea bargain",
        "literal": "Торг по признанию вины",
        "explanation": "Американская правовая практика: признаёшь вину — получаешь смягчение. Plea — заявление о виновности в суде, bargain — торг. ~95% уголовных дел в США закрываются именно так."
      },
      {
        "phrase": "A hedge",
        "literal": "Изгородь, защитный буфер",
        "explanation": "Перенос финансового термина в обычную речь: твой «страховочный план» на случай провала основного."
      }
    ]
  },
  {
    "id": 20,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 1,
    "context": "Куп (главный герой) закадровым голосом объясняет зрителю свою «бизнес-модель» — кражи у богатых соседей, которые часто даже не замечают пропажу драгоценностей и часов.",
    "quote_en": "Piles of forgotten wealth just lying around in drawers.",
    "quote_ru": "Кучи забытого богатства, просто валяющиеся в ящиках.",
    "expressions": [
      {
        "phrase": "Lying around",
        "literal": "Валяться вокруг",
        "explanation": "Фразовый глагол: вещи, оставленные без присмотра. To lie around — лежать беспорядочно, без дела."
      }
    ]
  },
  {
    "id": 21,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 2,
    "context": "Куп (главный герой) закадровым голосом описывает зрителю атмосферу званых ужинов и вечеринок в богатом пригороде Westmont, где он живёт.",
    "quote_en": "Great food, good company, and an endless parade of conspicuous consumerism.",
    "quote_ru": "Отличная еда, хорошая компания и бесконечный парад демонстративного потребления.",
    "expressions": [
      {
        "phrase": "Conspicuous consumerism",
        "literal": "Заметное потребление",
        "explanation": "Термин социолога Торстейна Веблена (1899, «Теория праздного класса»): покупка дорогих вещей не ради пользы, а ради демонстрации статуса. Веблен называл это формой социального соревнования."
      }
    ]
  },
  {
    "id": 22,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 3,
    "context": "Куп (главный герой) закадровым голосом объясняет зрителю, почему он начал красть редкие винтажные вина из погребов соседей — у них есть рынок коллекционеров.",
    "quote_en": "They're more than happy to fork out 32 grand a bottle for it.",
    "quote_ru": "Они с радостью раскошелятся на 32 штуки за бутылку.",
    "expressions": [
      {
        "phrase": "To fork out",
        "literal": "Выложить вилкой",
        "explanation": "Фразовый глагол XVIII в.: «выкладывать», как пальцы (вилка) разжимаются при отдаче денег. Всегда подразумевает неохотную или крупную трату."
      },
      {
        "phrase": "A grand",
        "literal": "Большой",
        "explanation": "Сленг для $1000, известен с 1915 г. (предположительно из американских преступных кругов). 32 grand = $32,000."
      },
      {
        "phrase": "More than happy to",
        "literal": "Более чем счастливы",
        "explanation": "Усилитель готовности: «с превеликим удовольствием». Часто используется иронически."
      }
    ]
  },
  {
    "id": 23,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 4,
    "context": "Лу (скупщица краденого, бывшая воровка) — Купу (главному герою), оценивая часы Patek Philippe, которые он только что украл у соседа.",
    "quote_en": "That watch is a serious piece of hardware.",
    "quote_ru": "Эти часы — серьёзный экземпляр.",
    "expressions": [
      {
        "phrase": "A piece of hardware",
        "literal": "Кусок железа",
        "explanation": "Hardware — технические штуки (в т.ч. оружие). Применительно к люксовым часам — намёк на сложную механику и весомость. Сленговое признание серьёзной вещи."
      }
    ]
  },
  {
    "id": 24,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 4,
    "context": "Куп (главный герой) закадровым голосом рассказывает зрителю про сумку Hermès Birkin — сейчас он планирует украсть такую у соседки Сюзанны.",
    "quote_en": "There's probably no more obnoxious or coveted status symbol than the Hermès Birkin.",
    "quote_ru": "Пожалуй, нет более вызывающего и желанного символа статуса, чем Hermès Birkin.",
    "expressions": [
      {
        "phrase": "Status symbol",
        "literal": "Символ статуса",
        "explanation": "Термин социолога Дж. Х. Фихтера (1944): вещь, демонстрирующая место в иерархии. Получил массовое употребление в эпоху послевоенного потребительского бума."
      },
      {
        "phrase": "Coveted",
        "literal": "Желанный, вожделенный",
        "explanation": "От библейской заповеди Thou shalt not covet («Не пожелай»). В современном английском — то, чего страстно хотят, но трудно получить."
      }
    ]
  },
  {
    "id": 25,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 4,
    "context": "Куп (главный герой) закадровым голосом объясняет зрителю, как работает система покупки сумки Hermès Birkin — нужно сначала потратить большие суммы на другие товары бренда, чтобы тебя поставили в лист ожидания.",
    "quote_en": "Spending tens of thousands on jewelry and accessories, aka Birkin bait.",
    "quote_ru": "Тратишь десятки тысяч на украшения и аксессуары — так называемую «приманку для Биркина».",
    "expressions": [
      {
        "phrase": "Birkin bait",
        "literal": "Приманка для Биркина",
        "explanation": "Реальная практика Hermès: чтобы получить сумку Birkin (лист ожидания + одобрение продавца), нужно сначала потратить большие суммы на другие товары бренда."
      },
      {
        "phrase": "AKA",
        "literal": "Также известно как",
        "explanation": "Аббревиатура от also known as. Используется для введения второго названия или прозвища."
      }
    ]
  },
  {
    "id": 26,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 3,
    "context": "Куп (главный герой) закадровым голосом размышляет о жителях своего богатого пригорода — у них уже есть всё, и в их жизни больше нет места переменам.",
    "quote_en": "This would be the sum total of their lives. Their futures are already written.",
    "quote_ru": "Это и будет итогом их жизни. Их будущее уже написано.",
    "expressions": [
      {
        "phrase": "The sum total of",
        "literal": "Сумма-итог",
        "explanation": "Усиленное «итого». Sum и total — синонимы, но вместе подчёркивают окончательность подсчёта. Часто в философско-итоговом контексте."
      },
      {
        "phrase": "Futures are already written",
        "literal": "Будущее уже написано",
        "explanation": "Метафора предопределённости (от Шекспира до арабской maktub — «так написано»). Намёк на детерминизм, отсутствие выбора."
      }
    ]
  },
  {
    "id": 27,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 3,
    "context": "Куп (главный герой) закадровым голосом размышляет, почему богатые жители его пригорода так отчаянно скупают вино, часы, машины — это попытка чем-то заполнить экзистенциальную пустоту.",
    "quote_en": "And so the quest to stave off the emptiness began.",
    "quote_ru": "И так начался поход против пустоты.",
    "expressions": [
      {
        "phrase": "To stave off",
        "literal": "Отбивать палкой",
        "explanation": "Stave — деревянная палка/посох. Старое значение «отбивать палкой нападающую собаку или зверя». В переносном смысле — отгонять что-то нежелательное (голод, скуку, банкротство)."
      },
      {
        "phrase": "Quest",
        "literal": "Поиск, странствие",
        "explanation": "От лат. quaerere — искать. Связано с рыцарскими поисками (Грааль, дракон). В современном английском придаёт повседневному действию эпическую окраску."
      }
    ]
  },
  {
    "id": 28,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 3,
    "context": "Куп (главный герой) закадровым голосом объясняет зрителю, почему люксовые бренды и винные дилеры так хорошо зарабатывают — они паразитируют на скуке и тревоге богатых соседей Купа.",
    "quote_en": "Cash in on the quiet desperation of rich, middle-aged men.",
    "quote_ru": "Нажиться на тихом отчаянии богатых мужчин среднего возраста.",
    "expressions": [
      {
        "phrase": "To cash in on",
        "literal": "Обналичить на",
        "explanation": "Фразовый глагол: использовать ситуацию для прибыли. Из карточных игр — to cash in chips (обменять фишки на деньги). Часто негативно: «нажиться»."
      },
      {
        "phrase": "Quiet desperation",
        "literal": "Тихое отчаяние",
        "explanation": "Фраза Генри Дэвида Торо из «Уолдена» (1854): «The mass of men lead lives of quiet desperation» — «Масса людей живёт жизнью тихого отчаяния». Стала клише про подавленное недовольство благополучных."
      }
    ]
  },
  {
    "id": 29,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 4,
    "context": "Лу (скупщица краденого) — Купу (главному герою) в своём подвале-приёмке, обсуждая судьбу украденной им редкой сумки Hermès Birkin лимитированной серии.",
    "quote_en": "Sure, we'll get a fat check, but it's a limited edition piece.",
    "quote_ru": "Конечно, мы получим жирный чек, но это эксклюзивная вещь.",
    "expressions": [
      {
        "phrase": "A fat check",
        "literal": "Жирный чек",
        "explanation": "Fat в значении «обильный, щедрый» — английское использование с XVII в. (a fat profit, a fat salary). Намёк на «толщину» суммы — много нулей."
      },
      {
        "phrase": "Limited edition",
        "literal": "Ограниченное издание",
        "explanation": "Маркетинговый термин XX в.: ограниченный тираж создаёт искусственный дефицит и поднимает цену. Активно использовался люксовыми брендами с 1960-х."
      }
    ]
  },
  {
    "id": 30,
    "show": "Your Friends & Neighbors",
    "season": 1,
    "episode": 7,
    "context": "Куп (главный герой) закадровым голосом описывает зрителю благотворительный гала-ужин в Westmont — место, где собираются богатые одинокие женщины, идеальные жертвы для его краж драгоценностей.",
    "quote_en": "It's a target-rich environment with a built-in spotlight.",
    "quote_ru": "Богатая на мишени среда — да ещё и со встроенной подсветкой.",
    "expressions": [
      {
        "phrase": "Target-rich environment",
        "literal": "Среда, богатая мишенями",
        "explanation": "Военный термин из американской авиации времён Вьетнама и Войны в Заливе: район, где много целей для удара. Перешло в гражданский сленг — место, где много возможностей (для продажника, охотника на богачей и т.д.)."
      },
      {
        "phrase": "Built-in",
        "literal": "Встроенный",
        "explanation": "Built (построен) + in (внутрь) = изначально включённый, не нуждающийся в добавлении. Часто про функции техники, но и переносно."
      }
    ]
  }
]